...
import { getStripeInvoiceFromCheckoutSession } from "@/src/entities/invoice";
import Link from "next/link";
import { useQueryState } from "nuqs";
import { Suspense, useEffect, useState } from "react";

function PaymentStatusContent() {
  const [sessionId] = useQueryState("invoice");
    ...
    (async () => {
      if (sessionId) {
        const { invoicePdf, invoiceUrl } =
          await getStripeInvoiceFromCheckoutSession(sessionId);
