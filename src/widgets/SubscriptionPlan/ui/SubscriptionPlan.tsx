...
import { getStripeSubscription, updateStripeSubscription } from "@/src/entities/subscription";
...
export const SubscriptionPlan = () => {
    ...
    const [selectedPlan, setSelectedPlan] = useState<null | { priceId: string, title: string, price: number }>(null)
    ...
    const selectUpdatedPlan = (plan: { priceId: string, title: string, price: number }) => {
        toggle()
    ...
    const updateSubscription = async () => {
        if (selectedPlan) {
            const result = await updateStripeSubscription({ customerId, priceId: selectedPlan.priceId })
    ...
    useEffect(() => {
        (async () => {
            ...
            if (userId) {
                ...
                const subscription = await getStripeSubscription({ customerId: user.customerId }) as Subscription
        ...
        <>
            <section className="bg-gray-900 dark:bg-gray-900 min-h-screen flex flex-col justify-center">
                ...
                <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                  ...
                  ${item.id === currentPlan ? 'cursor-not-allowed bg-gray-500' : 'bg-green-600 hover:bg-green-800 '}
                  text-white 
                  ...
                  `}
                                    onClick={() => currentPlan ? selectUpdatedPlan({ priceId: item.id, price: item.price, title: item.title }) : paymentSubscribe(item.id)}
                                    type="button"
                                    disabled={loading === item.id || item.id === currentPlan || init}
                                >
                                    {loading === item.id || init ? (
                                        ...
                                        item.id === currentPlan ? "Current plan" : currentPlan ? 'Update plan' : "Get started"
                                    )}
                                </button>
                            </div>
                        ))}
